...
const DoctorDashboard = () => {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      if (!doctorId) return;
      try {
        const res = await axios.get(`http://localhost:3000/api/appointments?doctorId=${doctorId}`);
  ...
  return (
    <DoctorLayout>
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 ">
      <div>
        <h2 className="text-2xl font-bold">Hello, {doctor?.name || "Unknown"}</h2>
        <p className="text-gray-500">Welcome Back</p>
        <p className="text-sm text-pink-600 mt-1 flex items-center gap-2 flex-wrap">
      ...
      <img
        ...
        alt="Doctor"
        className="h-24 sm:h-52 object-cover rounded-full border-2 border-pink-500"
      />
    </div>
    ...
    <div className="bg-gradient-to-r from-pink-100 to-purple-100 p-4 sm:p-6 rounded-lg shadow mb-6">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white rounded-lg shadow p-4 sm:p-6">
        <div className="flex justify-between items-center mb-2">
          <h4 className="font-bold text-lg">Appointments</h4>
        </div>
